...
CREATE TABLE IF NOT EXISTS `too_many_cookbooks`.`ingredient_recipe` (
  ...
  INDEX `fk_ingredients_has_recipe_ingredients_idx` (`ingredient_id` ASC),
...
CREATE TABLE IF NOT EXISTS `too_many_cookbooks`.`shopping_item` (
  ...
  `amount` INT NOT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_shopping_list_ingredient1_idx` (`ingredient_id` ASC),
...
CREATE TABLE IF NOT EXISTS `too_many_cookbooks`.`recipe_cookbook` (
  ...
  INDEX `fk_recipe_has_cookbooks_cookbooks1_idx` (`cookbook_id` ASC),
  INDEX `fk_recipe_has_cookbooks_recipe1_idx` (`recipe_id` ASC),
